...
export default function ExtractionResults({ jobId }: ExtractionResultsProps) {
          ...
          <div className="text-foreground text-sm">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {result.objects.map((obj, objIndex) => (
                      <div key={objIndex} className="border border-border rounded-md p-3 bg-muted/50" data-testid={`object-${objIndex}`}>
              ...
              {result.tables && result.tables.map((table, index) => (
                <div key={index}>
                  {table.title && <h3 className="font-semibold text-lg">## {table.title}</h3>}
                  {table.confidence && (
    ...
    <Card data-testid="card-extraction-results">
      ...
      <CardContent className="space-y-4">
        {job && (job.status === 'processing' || job.status === 'pending') && (
          <div className="space-y-3" data-testid="processing-status">
